...
export function InvoicesExport() {
    ...
    const doDownload = () => {
        if (splitFilesPerCompany) {
            selectedCompanies.forEach(({ company, companyTokenBatches }) => {
                const subscriptionsForCompany = subscriptions.filter(invoiceWithUser => selectedSubscriptions.includes(parseInt(invoiceWithUser.subscription_id)) && invoiceWithUser.company_id === company?.id);
                const csv = getCSV(subscriptionsForCompany, format, includeExcelStringFormat);
                console.log({ csv });
                downloadBlob(csv, (company?.name ?? "invoices") + ".csv", "text/csv;charset=utf-8");
            });
        } else {
            ...
            const csv = getCSV(selectedSubscriptionsForCompany, format, includeExcelStringFormat);
            ...
            <div className="flex flex-col justify-center items-center w-[400px] p-2">
                ...
                <div className="flex flex-col overflow-auto h-full w-full gap-4 my-3">
                    ...
                    <div className="flex flex-col gap-1">
                        <BooleanSwitchWithLabel label={"Min interactions"}
                                                size={"small"}
                                                value={minCompletedInteractions !== null}
                                                onValueChange={(checked) => {
...
function getCSV(data: SubscriptionWithUser[], format: "140A" | "standard", includeExcelStringFormat: boolean) {
    const BOM = "\uFEFF";
    if (format === "140A") {
        ...
        const downloadData = data.flatMap((invoiceWithUser) => createStandardEntriesFrom(invoiceWithUser, includeExcelStringFormat));
