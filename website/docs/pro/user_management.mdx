...
users and roles in a Firestore backend.
...
and permissions system directly into your Firestore backend. This plugin allows you to control which users can access
...
and optional settings such as user limits and permissions.

The default config of the plugin will be saved to your Firestore database under the path `__FIRECMS/config`,
...
import {useFirestoreUserManagement, useUserManagementPlugin, userManagementAdminViews} from "@firecms/user_management";
