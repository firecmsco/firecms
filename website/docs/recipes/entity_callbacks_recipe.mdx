...
The intreface of the collection will look like this:
...
We want to save the user who made the last change and the timestamp of the last change. We can use the `onPreSave` callback to achieve this. We are going use the builder util function `buildEntityCallbacks` to create the callbacks. This is helpful for creating the callbacks for a specific entity type. So we have the types inside the functions.
