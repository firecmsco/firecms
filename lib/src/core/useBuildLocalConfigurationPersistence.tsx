...
export function useBuildLocalConfigurationPersistence(): UserConfigurationPersistence {
    ...
    function onCollectionModified<M extends object>(path: string, data: LocalEntityCollection<M>) {
        const storageKey = `collection_config::${stripCollectionPath(path)}`;
        localStorage.setItem(storageKey, JSON.stringify(data));
    }

    function getCollectionConfig<M extends object>(path: string): LocalEntityCollection<M> {
        const storageKey = `collection_config::${stripCollectionPath(path)}`;
