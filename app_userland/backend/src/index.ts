...
// Load environment from app_userland root level
dotenv.config({ path: path.resolve(__dirname, "../../.env") });

const app = express();
const server = createServer(app);

// PostgreSQL connection — points to Firebase Data Connect Cloud SQL
...
async function startServer() {
    ...
    const backend = await initializeRebaseBackend({
        ...
        auth: {
            ...
            email: process.env.SMTP_HOST ? {
                ...
                appName: process.env.APP_NAME || "Rebase Userland",
                resetPasswordUrl: process.env.FRONTEND_URL || "http://localhost:5174",
                verifyEmailUrl: process.env.FRONTEND_URL || "http://localhost:5174",
                smtp: {
    ...
    server.listen(PORT, () => {
        console.log(`🚀 Userland server running at http://localhost:${PORT}`);
        console.log(`🗄️ Connected to Firebase Data Connect Cloud SQL`);
