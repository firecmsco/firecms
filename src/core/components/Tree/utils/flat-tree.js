...
export const getDestinationPath = (flattenedTree, sourceIndex, destinationIndex, level) => {
  // Moving down
  const down = destinationIndex > sourceIndex; // Path of the source location

  const sourcePath = getSourcePath(flattenedTree, sourceIndex); // Stayed at the same place

  const sameIndex = destinationIndex === sourceIndex; // Path of the upper item where the item was dropped

  const upperPath = down ? flattenedTree[destinationIndex].path : flattenedTree[destinationIndex - 1] && flattenedTree[destinationIndex - 1].path; // Path of the lower item where the item was dropped

  const lowerPath = down || sameIndex ? flattenedTree[destinationIndex + 1] && flattenedTree[destinationIndex + 1].path : flattenedTree[destinationIndex].path;
  /*
  ...
  */
  ...
  if (upperPath) {
    ...
    const finalLevel = calculateFinalLevel(sourcePath, upperPath, lowerPath, level); // Insert to higher levels
