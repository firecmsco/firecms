...
export default defineConfig({
    ...
    integrations: [
        ...
        mdx(),
        sitemap({
            // The API reference under /docs/api is generated by TypeDoc and is
            // English-only, but Starlight's i18n routing still emits an identical
            // copy of every page under /es, /de, /fr, /it and /pt. That is 2,335
            // duplicate URLs — 62% of the whole sitemap — spending crawl budget on
            // untranslated machine-generated content that earns a 0.40% CTR, while
            // the localised marketing pages that actually convert wait behind them.
            // The English originals stay in the sitemap; the translated duplicates
            // are dropped. Nothing is blocked or removed, only de-prioritised.
            filter: (page) => !/\/(es|de|fr|it|pt)\/docs\/api\//.test(page),
            i18n: {
                defaultLocale: 'en',
                locales: {
                    en: 'en',
