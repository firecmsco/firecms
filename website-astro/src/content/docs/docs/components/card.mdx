...
Import the `Card` from `@rebasepro/ui` and wrap the content you wish to display on the card. You can also make the card clickable by providing an `onClick` handler.
