...
description: Learn how to dynamically generate navigation and collections from database documents in Rebase.
---
...
Typically in Rebase you pass a static list of collections to the main CMS
component, but in this case we need to build the collections dynamically based
on the data in the database.

Rebase allows you to pass a function that returns a list of collections to the
`collections` prop of the `RebaseApp` component.
