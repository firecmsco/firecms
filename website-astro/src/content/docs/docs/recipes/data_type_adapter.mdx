...
description: How to create a custom adapter for a data type in Rebase
---

:::info
In order to use this feature, you need to initialize a Rebase repository. You can do this by following the
[Customization quickstart](../cloud/quickstart) guide.
:::

Rebase allows you to customize the way data is displayed and edited in the CMS. By default, the views created by
Rebase are based on the data type defined in your collection properties:
...
```typescript
import {EntityCallbacks, EntityOnFetchProps} from "@rebasepro/core";
...
```typescript
import {EntityCallbacks} from "@rebasepro/core";
...
```typescript
import {buildCollection, EntityCallbacks, EntityCollection, EntityOnFetchProps} from "@rebasepro/core";
