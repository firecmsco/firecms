...
title: Styling Rebase
description: Rebase allows you to customize the look and feel of your admin panel. You can customize the theme, colors, and typography to match your brand.
---

Rebase allows you to customize the look and feel of your admin panel. You can
customize the theme, colors, and typography to match your brand.

Rebase uses tailwindcss for styling. Rebase UI provides a default **preset** that
...
You may want to check the original [`tailwind.config.js`](https://github.com/rebasepro/rebase/blob/main/packages/ui/tailwind.config.js)
file in the `@rebasepro/ui` package.
...
Rebase UI uses a primary color and a secondary color. You can customize these colors
...
[`index.css`](https://github.com/rebasepro/rebase/blob/main/packages/ui/src/index.css) file in the `@rebasepro/ui` package.
...
color used by Rebase.

Rebase defines colors for all the surfaces (a light gray by default), surfaces accents (a bluish gray by default),
...
Rebase uses the `Rubik` font by default, both for headings and body text. It also uses the `JetBrains Mono` font
for code blocks.
...
In the Rebase UI tailwind preset, we define 3 different font families: `font-headers`, `font-sans` and `font-mono`.
...
If you need to customize any other aspect of Rebase, please [let us know](https://discord.gg/fxy7xsQm3m).
