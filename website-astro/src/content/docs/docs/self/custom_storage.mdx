...
title: Defining Custom Storage in Rebase
sidebar_label: Custom Storage Solutions
description: Implement custom storage solutions like Amazon S3 in Rebase with this step-by-step guide.
---

Rebase provides flexibility when it comes to integrating custom storage solutions.
While it comes with a built-in storage system that works through the backend API, you might want to define your
...
A custom storage source in Rebase is defined by implementing the `StorageSource` interface.
...
```tsx
import {StorageSource, UploadFileProps, UploadFileResult, DownloadConfig} from "@rebasepro/core";
...
After creating the custom storage source, you can use it in your Rebase application by initializing it in your component and passing it to the `Rebase` component.

### Example Usage

Here is an example of how to use the custom storage source in your Rebase application
...
const CustomStorageApp: React.FC = () => {
  const name = "My Custom Storage Rebase App";

  const modeController = useBuildModeController();
  const userConfigPersistence = useBuildLocalConfigurationPersistence();
  const storageSource = useCustomStorageSource(customStorageConfig);

  // const authController = useAuthController(); // your auth controller
  // const dataSource = {}; // Your data source delegate implementation
...
This documentation provides a clear guide for defining custom storage solutions in Rebase. Follow the template to integrate other storage services as per your requirements.
