...
This migration guide applies to **self-hosted versions** of Rebase, including both **Community** and **PRO** editions.
Rebase v3.1 introduces important updates that require configuration changes and offers exciting new features.

## Before You Start

- Ensure you are currently on **Rebase v3.0.x**
- Back up your project or commit your current state to version control
- Ensure you are using **Node.js 18+**

## Update Rebase Packages

Update all `@rebasepro/*` packages to version **3.1**:

```bash
npm i @rebasepro/core@3.1.0 @rebasepro/ui@3.1.0 @rebasepro/firebase@3.1.0 @rebasepro/collection_editor@3.1.0 @rebasepro/collection_editor_firebase@3.1.0 @rebasepro/data_enhancement@3.1.0 @rebasepro/data_export@3.1.0 @rebasepro/data_import@3.1.0 @rebasepro/schema_inference@3.1.0 @rebasepro/user_management@3.1.0
```

Make sure to include **all** `@rebasepro/*` packages listed in your `package.json`. Mixing v3.0 and v3.1 packages will cause issues.
...
:::warning
If you skip this step, your custom theme colors will **not be applied** and Rebase will fall back to its default palette.
...
Rebase v3.1 introduces new ways to visualize your collection data beyond the traditional table view.
...
Rebase v3.1 includes numerous bug fixes and improvements:
...
Rebase v3.1 brings AI-powered features to self-hosted PRO users that were previously only available in Rebase Cloud.
...
The `buildCollectionGenerationCallback` helper uses the default Rebase API endpoint. You can optionally provide a custom endpoint:
...
const dataTalkConfig = useBuildDataTalkConfig({
    ...
    firebaseApp,
    userSessionsPath: userEmail ? `__REBASE/config/users/${userEmail}/datatalk_sessions` : undefined,
    getAuthToken: authController.getAuthToken,
    loadSamplePrompts: true
    // apiEndpoint defaults to https://api.rebase.pro
});
...
return (
    <DataTalkProvider config={dataTalkConfig}>
        <Rebase navigationController={navigationController} /* ... */ >
            {/* your app */}
        </Rebase>
    </DataTalkProvider>
);
...
:::note
AI features require a valid Rebase PRO license. The API endpoints authenticate using your Firebase auth token.
:::
...
- Verify your `index.css` includes the `@source` directives for your project files and `node_modules/@rebase`
...
Ensure **all** `@rebasepro/*` packages are on the same version. Mixing v3.0 and v3.1 packages will cause type mismatches and runtime errors.
