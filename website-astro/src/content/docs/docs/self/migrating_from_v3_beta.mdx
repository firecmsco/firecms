...
description: Step-by-step migration guide from Rebase v3 beta versions to the stable release, covering PRO and Cloud changes.
---

## Rebase PRO

If you are migrating from previous beta versions of Rebase PRO, you will need to make some updates to your project.
...
You also will need to import the default Rebase styles in your project. You can do this by adding the following import to your `index.css` file:
...
If you are migrating from previous beta versions of Rebase Cloud, you will need to make some updates to your project.

### Dependencies

The main package has been renamed from `rebase` to `@rebasepro/cloud` since version `3.0.0-beta.4`.
You can also remove the `@rebasepro/cli` package, as it is implicitly installed by `@rebasepro/cloud`.
...
```
{
  ...
  "scripts": {
    ...
    "deploy": "run-s build && rebase deploy --project=YOUR_PROJECT_ID"
  },
  "dependencies": {
    "@rebasepro/cloud": "^3.0.0-beta",
    "firebase": "^12.0.0",
    "react": "^18.3.1",
    "react-dom": "^18.3.1"
  },
...
You need to update your imports to use the new package name. For example, if you have a file that imports `rebase`:
...
- The preset now comes from the `@rebasepro/ui` package. You can update your `tailwind.config.cjs` file to include the new preset:
