...
title: What's new in Rebase 3.0
sidebar_label: What's new in Rebase 3.0
description: Rebase 3.0 includes pioneering features and enhancements for a dynamic and customizable content management experience. Now integrated with a backend, it extends the flexibility of collection schema modification directly from the UI, streamlining the workflow for end users. Innovations include an intuitive UI schema editor, intelligent data inference for quick setup, and advanced data import/export capabilities. Notably, the switch to Tailwind CSS marks a leap in performance, delivering a swift, more efficient CMS interface. Rebase 3.0 transforms content management with its blend of user-friendly features and performance optimizations, setting a new standard in headless CMS solutions.
---

The new version of Rebase is a major release that brings a lot of new features
and improvements. This page will guide you through the most important changes 🔥

Rebase 3.0 comes in three flavors: **Rebase Cloud**, **Rebase Community** and **Rebase PRO**.

## Rebase Cloud

Rebase Cloud makes now use of a **backend** to provide a more flexible and
customizable experience. This backend is hosted by us, and you don't have to
worry about it. You can still use Rebase Cloud for free, but you will have to
pay for some features.

Until now, Rebase was a frontend-only library that would connect directly to
...
Rebase PRO is a version of Rebase that you can host yourself. This means that
you have full control over your data, and you can customize the CMS to your
needs. You can try out Rebase PRO for free, but you will need a license to use.

Rebase PRO is great for large projects, or when you need to customize the CMS
to your needs. You can achieve the highest level of customization of all previous Rebase versions.

Rebase PRO is perfect for agencies that are looking to provide a CMS to their clients.
...
to get a demo of Rebase PRO.


## Rebase Community

Rebase Community is a free and open source version of Rebase that you can
host yourself. This means that you have full control over your data, and you can
customize the CMS to your needs.

MIT licensed and completely free, Rebase Community is a great option for
small projects that just need a CMS. It does not include all the bells and whistles
of Rebase Cloud or Rebase PRO, but it is still a powerful alternative, due to
its customizability and flexibility.

This version has all the functionality of Rebase 2.0 but with many of the improvements
...
In Rebase Cloud, **the collection schema is stored in Rebase backend**, but you are also able to define
...
editor and add the new value. Even better, Rebase can find new values and add
...
quickly? Rebase can now **infer the schema from your data**. This means that
you can get started with Rebase in a few minutes, without having to write a
single line of code.

## Local text-based search

In previous versions of Rebase, you could add your external search engine solution
...
Versions 1.0 and 2.0 of Rebase were based on Material UI (mui). This was great
...
In Rebase 3.0 we have migrated to Tailwind CSS. This is a utility-first CSS
...
a collection. Rebase UI now exports all the Material Icons, conveniently exported as React components.
