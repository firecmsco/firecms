...
title: Migrating from Rebase 2.0 to Rebase Cloud
sidebar_label: Migrating from Rebase 2.0 to Rebase Cloud
description: Upgrade to the modern **React-based Firebase CMS**. Migrate your admin panel to Rebase Cloud for better performance and features.
---

:::important
This migration guide applies for migrating from Rebase 2.0 to Rebase Cloud
:::
...
## Create a project in app.rebase.pro

Rebase Cloud requires the creation of a project in [app.rebase.pro](https://app.rebase.pro).
...
## Initialize a Rebase Cloud project in a new folder
...
Despite the new format, Rebase aims allow users to migrate existing apps with minimal
changes. The collections can be now stored both in the Rebase backend or defined in
code like until now.
...
For reference, the `package.json` file of a new Rebase Cloud project looks like this:

```json
{
  "name": "my-rebase-project",
  "private": true,
  "version": "1.0.0",
  "type": "module",
  "scripts": {
    ...
    "deploy": "run-s build && rebase deploy --project=your-project-id"
  },
  "dependencies": {
    "@rebasepro/cloud": "^3.0.0",
    "firebase": "^12.0.0",
    "react": "^18.3.1",
    "react-dom": "^18.3.1"
  },
...
Since it is now possible to deploy Rebase in our hosted service, the output
of your project needs to be in a specific format.

The `index.ts` file should export a `RebaseAppConfig` object, which is defined as follows:
...
- `CircularProgress` size is a string instead of a number. You can use `size="small"` or ``.

### Deployment

Rebase 3.0 is now deployed in our own service, and reachable through [app.rebase.pro](https://app.rebase.pro).
