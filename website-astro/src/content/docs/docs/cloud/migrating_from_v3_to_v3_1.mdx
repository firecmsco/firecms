...
description: Upgrade your Rebase Cloud custom code to v3.1 — Tailwind CSS v4, React 19, and new collection views.
---

This migration guide applies to **Rebase Cloud** projects that deploy custom code using the CLI
(`rebase deploy`).

## Do I Need to Do Anything?

**All Rebase Cloud users get every new feature automatically** — Kanban board, Cards view,
AI collection generation, and more are already live in your project. No update needed.

If you deploy **custom code** via the CLI (`rebase deploy`), your existing build will continue to
...
Update all `@rebasepro/*` packages in your `package.json` to version **3.1**:

```bash
npm i @rebasepro/cloud@3.1.0
```
...
Rebase 3.1 ships with **Tailwind CSS v4** support. While your existing Tailwind v3 setup will
...
Rebase 3.1 supports **React 19** while maintaining backward compatibility with React 18.3.
...
These features are already available in your Rebase Cloud project — no update required:
...
Ensure **all** `@rebasepro/*` packages are on the same version. Mixing v3.0 and v3.1 packages will
