---
slug: docs/cloud/index
title: "Rebase Cloud - The Ultimate Firebase Admin Panel and Headless CMS"
sidebar_label: Rebase Cloud
description: "A powerful Firebase admin panel and headless CMS for Firestore. Manage data, users, and content with a beautiful React-based UI. No-code ready with full code customization."
---


import { DocsCloudCTA } from "../../../../components/DocsCloudCTA.tsx";


:::tip
Rebase Cloud is the **ultimate Firebase admin panel** and **headless CMS**.
It provides a fully managed, **React-based back-office** that allows you to manage your Firestore data, users, and content with ease.

**No-code ready**: Visualize and edit data immediately.
**Code enabled**: Upload custom React code, entity views, and property builders.
:::

## Why Rebase Cloud?

Building a custom **admin panel** or **internal tool** from scratch is time-consuming. Rebase Cloud gives you a production-ready **CMS** in minutes, tailored for **Firebase** and **Firestore**.

### Key Features

- **Managed Service**: We handle hosting, scalability, and updates. You focus on your data and business logic.
- **Beautiful UI**: A modern, responsive interface for your **Firestore database**.
- **Collection Editor**: Define your schema visually with our no-code editor.
- **Developer Friendly**: Extend with **React**. Create custom form fields, dashboard widgets, and previews.
- **Role-Based Access**: Granular permission control for your content team and administrators.


## Getting started

You can use Rebase Cloud with your existing Firestore project, or let it create
a new one for you.

Just sign up and start using it!

<DocsCloudCTA/>

After you have created your Rebase project, you can either use it directly or [customize it
by uploading your own code](./quickstart).

