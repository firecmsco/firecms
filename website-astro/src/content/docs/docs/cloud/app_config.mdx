...
title: App Config
description: A comprehensive guide to configuring Rebase, a customizable CMS. It provides detailed instructions on how to initialize a new project, customize the CMS with reusable properties, fields, collections, entity views, and CMS views. It also explains the customization options available, including collections, views, propertyConfigs, entityViews, HomePage, dateTimeFormat, locale, textSearchController, and autoOpenDrawer. This page is a valuable resource for developers seeking to tailor Rebase to their specific needs.
---

The app config is the main configuration object of Rebase Cloud. It is defined
by the interface `RebaseAppConfig`. In order to customize the CMS, you need
to create a project in [app.rebase.pro](https://app.rebase.pro) and
initialise a new project in code with `npx create-rebase-app` or `yarn create rebase-app`.

After those steps are ready you can use the `RebaseAppConfig` export to add
...
* `rebaseAppBarComponentProps`: Additional props passed to the custom AppBar.
* `indexesBuilder`: Use this builder to indicate which indexes are available in your Postgres database.
    This is used to optimize filtering and sorting for multiple fields in the CMS.
* `onInit`: Optional callback after the CMS has been initialised.
    Useful for custom initialization or retrieving the used configuration.
* `dateTimeFormat`: Format of the dates in the CMS.
    Defaults to 'MMMM dd, yyyy, HH:mm:ss'.
* `locale`: Locale of the CMS, currently only affecting dates.
* `textSearchController`: Use this controller to return text search results as document ids, that get then fetched from the database.
