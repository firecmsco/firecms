...
This plugin adds a history view to your entities in Rebase. It allows you to track changes made to entities over time, showing who made the change and when.
...
To use the plugin, import `useEntityHistoryPlugin` and add it to your plugins array.
...
```tsx
import { createHistoryEntry, NewHistoryEntryParams } from "@rebasepro/entity_history";

// Example: Creating a history entry when importing data
const handleDataImport = async (context: RebaseContext, importedData: any[]) => {
...
const handleCustomUpdate = async (context: RebaseContext, entityId: string, oldValues: any, newValues: any) => {
...
| ---------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `context`        | `RebaseContext<User>`    | The Rebase context object containing the data source and auth controller                                                                        |
...
Each history entry is automatically stored in a `__history` table linked to the entity and includes:
