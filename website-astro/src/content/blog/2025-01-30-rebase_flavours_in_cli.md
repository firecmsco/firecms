---
slug: rebase_flavours_in_cli
title: Rebase Flavours in the CLI
description: Kickstart a new Rebase project in a single command with our new CLI flavours.
pubDate: 2025-01-30
image: /img/blog/demo_cli.png
authors: marian
---

![Demo CLI](/img/blog/demo_cli.png)

## Kickstart a new project with Rebase in a single command
...
That's it! You are ready to go. You can now start building your project with [Rebase](https://rebase.pro).

### Rebase Flavors

If you are familiar with Rebase, you must already know the [different flavors](https://rebase.pro/pricing) we offer: **Community**, **Cloud**, or **Pro**. Or maybe we just confused you a little in the past. No worries, let's recap.

#### Community

The **classic** old-fashioned MIT licensed **Rebase**. You can use it for free, forever. It's the **perfect** choice for **small projects** or personal use. You handle the hosting; we handle the CMS. Create collections, add authentication, set it up, and after a working build, deploy it to Firebase Hosting or the hosting of your choice.
...
We expect this CLI to be less used than the Community one, but if you want to do some work from the code, it is up to you. We truly hope that the tools we've been building on [app.rebase.pro](https://app.rebase.pro) will be your main tool.

#### Pro

The **ultimate** version of **Rebase**. The Cloud version with a custom domain, custom branding, and priority support.
...
What can you expect? A schema editor and data inference, advanced data import and export, Notion-style editor, local text search, user and role management, custom authentication and access control, access to all Rebase plugins, full CMS components customization, priority support, and roadmap prioritization.

#### Flavors compared

Who doesn't like a good comparison table? Here is a quick overview of the features you get with each flavor:

| Feature                                | Community | [Cloud](https://app.rebase.pro) | Pro |
|----------------------------------------|-----------|---------------------------------|-----|
...
| Access to All Rebase Plugins          | ❌ | ❌                               | ✅ |

### Installation

If this is your first time using Rebase, you need to install it first. You have two choices: either you install the Rebase CLI globally with your desired package manager:
```bash
npm install -g rebase
# yarn fan?
yarn global add rebase
# pnpm? Sure!
pnpm add -g rebase
```
...
Ok, let's get back to the CLI. You've already installed it, right? Now you can start a new project with Rebase. Just run:

```bash
marian@m ~ % rebase init
...
Welcome to the Rebase CLI 🔥

You are logged in as hello@example.com
? Choose a template
  Rebase Cloud (use this option if you access Rebase from app.rebase.pro)
❯ Rebase PRO (self-hosted version with full functionality)
  Rebase PRO with Next.js frontend(self-hosted version with frontend boilerplate CRUD app)
  Rebase Community (MIT licensed version with base functionality)
```

After carefully choosing the flavor you want to use, you will get a new project with Rebase. Completely set up and ready to go.
...
```bash
npm install -g rebase@latest
```

### Other issues and feedback

Drop us an email at [hello@rebase.pro](mailto:hello@rebase.pro) or even better, join our [Discord server](https://discord.gg/fxy7xsQm3m) and let us know about your issue. We are always happy to help.

### Conclusion

We hope the new Rebase CLI brings you joy and saves you time.
