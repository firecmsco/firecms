...
title: "Beyond the Table: Cards and Kanban Views in Rebase"
description: Rebase 3.1 introduces Cards and Kanban views - pick the interface that actually makes sense for your data
pubDate: 2026-02-02
authors: marian
...
**Rows and columns. Cells and headers.** When we built Rebase, the table view made sense — it's familiar, it's dense, it handles data well. And for many use cases, it's still the right choice. But after years of using our own product, we kept hitting the same walls.
...
Rebase 3.0 introduces **Cards View** and **Kanban View** — two new ways to visualize your collections that we've wanted for a long time. The table view isn't going anywhere (it's still the best choice for dense data), but now you have options.
...
Rebase now has three visualization modes:
...
| **Kanban** | Workflows, pipelines, status tracking | Trello built into Rebase |
...
The `orderProperty` must reference a number field. Rebase uses fractional indexing to maintain order without rewriting every document on each reorder.
...
On **[Rebase Cloud](https://app.rebase.pro)**? You can configure all of this through the UI. No TypeScript needed.
...
Update to the latest Rebase:

```bash
npm install rebase@latest
```

Already on Rebase Cloud? Head to your collection settings and pick a view mode — it's already there.

Questions? Join us on [Discord](https://discord.gg/fxy7xsQm3m) or check the [docs](https://rebase.pro/docs).
