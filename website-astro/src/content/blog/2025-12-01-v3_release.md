---
slug: v3_release
title: Rebase 3.0 is here
description: Rebase 3.0 is our biggest release yet. A faster, more opinionated CMS for Firebase projects, with a new backend, UI, schema editor, import/export, and a lot of in‑house magic.
pubDate: 2025-12-01
authors: francesco
image: /img/rebase_logo.svg
---

> Rebase 3.0 is here and it is our biggest release so far.
>
> Rebase is growing into what it was always meant to be for Firebase teams.
> 
![Users table example](../../../public/img/users_table.png)

When we started Rebase, we wanted something very specific:
...
Over the years, thousands of companies have used Rebase to manage production
...
Rebase 3.0 is the version where all that experience is baked into the product.
...
But more importantly: 3.0 doubles down on what makes Rebase special.
...
Rebase is different because it is **deeply opinionated around Firebase and
...
Then Rebase is designed for you.
...
Instead of forcing Firestore into a generic CMS data model, Rebase leans into
it. That is why teams often tell us they can "think in Firestore" while using
Rebase.

### Thousands of teams, many unexpected use cases

We originally built Rebase to scratch our own itch at our development agency.
...
strong defaults. Rebase 3.0 is our answer to years of feedback from those
projects.
...
With Rebase 3.0 we have doubled down on building **our own primitives**:

- A **table and form system** that understands collections, nested objects,
  references and validation.
- A component library, [`@rebasepro/ui`](https://www.npmjs.com/package/@rebasepro/ui),
...
If you want to build your own views, you can import `@rebasepro/ui` and reuse the
...
Different teams need different levels of control. With Rebase 3.0, we now have
three flavors, all sharing the same foundations.

### Rebase Cloud: hosted, collaborative, Firebase‑first

Rebase Cloud is the easiest way to start: connect it to your Firebase project
and you get a fully hosted CMS with a **dedicated backend**.

Until now, Rebase was a frontend‑only library talking directly to Firebase
...
![Custom branded Rebase instance](../../../public/img/toleroo_recipe.webp)

### Rebase PRO: self‑hosted power

Rebase PRO is for teams that need all the advanced features, but need to own the
infrastructure:

- You host the backend.
- You keep full control over data residency and compliance.
- You get the most powerful way to customize every layer of Rebase.
...
### Rebase Community: open source core

The Community edition remains **MIT‑licensed and self‑hostable**.

It includes the full feature set of Rebase 2.0 plus many of the **UI and
...
In Rebase Cloud and PRO, schemas can live in the backend. That unlocks a
...
Rebase 3.0 can **infer schemas from your existing data**, giving you:

- A first draft of your content model in minutes.
- A concrete starting point you can refine instead of inventing fields from
  scratch.

![Data import mapping in Rebase](../../../public/img/import_mapping.webp)

## Working with your data should feel effortless

We see Rebase as the control center for your Firebase project. That means
...
In Rebase 3.0, rich text editing has been reimagined around a **Notion‑like
experience**:
...
content editing inside Rebase feel natural enough that your team forgets they
...
Rebase 3.0 adds **local text‑based search**:
...
and keep using Rebase as the UI.
...
Versions 1.x and 2.x of Rebase used Material UI. It helped us move fast early
on, but it also meant runtime styling and heavier bundles.

In Rebase 3.0 we rebuilt the UI on **Tailwind CSS** and our own component
library:
...
- A **new logo** that better represents Rebase today.
- A redesigned **landing page** aligned with the product experience.

The goal is simple: Rebase should feel as good to use as the products you’re
building with it.
...
Rebase 3.0 introduces a new authentication system focused on **CMS users and
roles**:
...
All this sits on top of a big internal refactor that makes Rebase easier to
maintain and extend—both for us and for you if you are building on top of it.

## Where to go from here

If you are new to Rebase, the easiest way to start is:

1. Spin up **Rebase Cloud** and connect it to a Firebase project.
...
If you are already using Rebase 2.x, check out
[What’s new in Rebase 3.0](/docs/what_is_new_v3) for a more detailed breakdown
of the changes.

And if you are an agency or a team that needs more control, you can always
[book a demo of Rebase PRO](https://calendar.google.com/calendar/u/0/appointments/schedules/AcZssZ0INW8ihjQ90S4gkdo8_rbL_Zx7gagZShLIpHyW43zDXkQDPole6a1coo1sT2O6Gl05X8lxFDlp?gv=true)
to see what a self‑hosted setup looks like.

## Thank you

Rebase 3.0 is the sum of many bug reports, feature requests, emails, Discord
messages and PRs.

If you are already using Rebase, thank you for trusting it with your projects.
