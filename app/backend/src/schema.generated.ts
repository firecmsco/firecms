...
import { primaryKey, pgTable, integer, varchar, boolean, timestamp, jsonb, pgEnum, numeric, foreignKey, index, serial } from "drizzle-orm/pg-core";
...
export const implementosRelations = drizzleRelations(implementos, ({ many }) => ({
	maquinaria: many(maquinariaImplementos)
}));
...
export const maquinariaRelations = drizzleRelations(maquinaria, ({ many }) => ({
	...
	horas: many(horas),
	incidencias: many(incidencias),
	implementos: many(maquinariaImplementos)
}));
