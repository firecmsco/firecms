...
describe("generateDrizzleSchema", () => {
    ...
    it("should generate a view for a joinPath relation", async () => {
        const usersCollection: EntityCollection = {
        ...
        const imagesCollection: EntityCollection = {
        ...
        // Add a relation from users to images through posts
        const usersWithPostImages: EntityCollection = {
            ...usersCollection,
            relations: [
                ...(usersCollection.relations || []),
                {
                    relationName: "post_images",
                    target: () => imagesCollection,
                    cardinality: "many",
                    joinPath: [
                        { table: "posts", on: { from: "id", to: "author_id" } },
                        { table: "images", on: { from: "id", to: "post_id" } }
                    ]
                }
            ]
        };

        const result = await generateSchema([usersWithPostImages, postsCollection, imagesCollection]);
        const cleanResult = cleanSchema(result);

        expect(cleanResult).toContain('export const viewUsersToImages = pgTable("view_users_to_images"');
        expect(cleanResult).toContain('source_id: integer("source_id")');
        expect(cleanResult).toContain('post_id: integer("post_id").references(() => posts.id, { onDelete: "set null" })');

        const expectedRelation = `export const usersRelations = drizzleRelations(users, ({ one, many }) => ({ post_images: many(viewUsersToImages, { relationName: "post_images" }) }));`;
        expect(cleanResult).toContain(cleanSchema(expectedRelation));

        // Clean the result for SQL check by replacing all whitespace characters and the middle dot with a single space
        const cleanSqlResult = result.replace(/[\s·]+/g, " ");
        expect(cleanSqlResult).toContain("CREATE OR REPLACE VIEW view_users_to_images");
        expect(cleanSqlResult).toContain("SELECT users.id AS source_id, images.*");
        expect(cleanSqlResult).toContain("FROM users JOIN posts ON users.id = posts.author_id JOIN images ON posts.id = images.post_id");
    });
});
