...
const getDrizzleColumn = (propName: string, prop: Property, collection: EntityCollection): string => {
    ...
    switch (prop.type) {
        ...
        case "reference": {
            ...
            const collections = (global as any).loadedCollections as EntityCollection[] | undefined;
            if (!collections) {
                console.warn("Could not find loaded collections in global scope. Skipping reference type resolution.");
                return "";
            }

            const targetCollectionPath = resolveTargetTableName(refProp.path ?? (refProp as any).target);
...
const generateSchema = async (collections: EntityCollection[], outputPath?: string) => {
    ...
    schemaContent += "import { serial, pgTable, integer, varchar, boolean, timestamp, jsonb, pgEnum, numeric, foreignKey, index } from \"drizzle-orm/pg-core\";\n";
    ...
    const arrayTables = new Map<string, {
            ...
            Object.entries(collection.relations).forEach(([, relation]) => {
                if (relation.type === "manyToMany") {
                    try {
                        ...
                        const targetCollection = (relation.target as () => EntityCollection)();
                        const junctionTableName = resolveJunctionTableName(relation.through as any, sourceCollection, targetCollection);
                        ...
                        if (!junctionTables.includes(junctionTableName)) {
                            ...
                            const junctionCollection = (relation.through.table as () => EntityCollection)();
                            ...
                            const sourceJunctionColumns = getJunctionKeyColumns(relation.through.sourceJunctionKey, sourceCollection, sourceRefOptionsString);
                            const targetJunctionColumns = getJunctionKeyColumns(relation.through.targetJunctionKey, targetCollection, targetRefOptionsString);

                            schemaContent += `export const ${junctionTableVarName} = pgTable("${junctionTableName}", {\n`;
                            schemaContent += getDrizzleIdColumn(junctionCollection) + ",\n";
            ...
            if (collection.relations) {
                Object.entries(collection.relations).forEach(([, relation]) => {
                    if (relation.type === "manyToMany") {
                        const sourceCollection = collection;
                        const targetCollection = (relation.target as () => EntityCollection)();
                        const currentJunctionTableName = resolveJunctionTableName(relation.through as any, sourceCollection, targetCollection);
    ...
    relationsToGenerate.forEach(({
        ...
        schemaContent += `export const ${tableVarName}Relations = relations(${tableVarName}, (${helpers}) => ({\n${relationStr}\n}));\n\n`;
    });
...
const runGeneration = async (collectionsFilePath?: string, outputPath?: string) => {
    try {
        ...
        (global as any).loadedCollections = collections;
