 ...
 */
export function registerProjectTools(server: McpServer, api: RebaseApiClient) {

    server.registerTool(
        "list_projects",
        {
            description: "List all Rebase Cloud projects accessible by the authenticated user",
        },
        async () => {
            try {
    ...
    server.registerTool(
        ...
        {
            description: "List all Firestore root-level collections in a Rebase project. Returns collection paths and database IDs.",
            inputSchema: {
