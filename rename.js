...
function walk(dir, callback) {
    ...
    fs.readdirSync(dir).forEach(f => {
        ...
        try {
            ...
            if (isDirectory) {
                if (f !== 'node_modules' && f !== 'dist' && f !== '.next' && !f.startsWith('.') && f !== 'build') {
                    walk(dirPath, callback);
                }
            } else {
                if (f.endsWith('.ts') || f.endsWith('.tsx') || f.endsWith('.md') || f.endsWith('.mdx') || f.endsWith('.txt')) {
                    callback(dirPath);
                }
            }
        } catch (e) {}
...
function replaceInFiles() {
    ['packages', 'website-astro'].forEach(dir => {
        if (!fs.existsSync(dir)) return;
        walk(dir, file => {
